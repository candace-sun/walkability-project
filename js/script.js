function stateIdToAbbr(id) {
  ...
  function createVis(data, us, currentVariable, title, containerSelector) {
    // 1) Prepare projection & geoPath
    const projection = d3.geoAlbersUsa().scale(1300).translate([487.5, 305]);
    const path       = d3.geoPath().projection(projection);
    const counties   = topojson.feature(us, us.objects.counties).features;
    const container  = d3.select(containerSelector);
  
    // 2) Build wrapper DIV + SVG + one <g> (we'll pan/zoom this <g>)
    ...
    // 3) Join your CSV data into the county geometries
    counties.forEach(c => {
      const idStr = String(+c.id);
      if (data[idStr]) {
        c.properties = { ...c.properties, ...data[idStr] };
      } else {
        c.properties.missing = true;
      }
    });
  
    // 4) Build a colorScale for this variable
    const extent = d3.extent(counties, d =>
      d.properties[currentVariable] != null ? d.properties[currentVariable] : null
    );
    const palette = {
      walkability_index:    d3.interpolateYlGnBu,
      intersection_density: d3.interpolateReds,
    ...
    const colorScale = d3.scaleSequential()
      .domain(extent)
      .interpolator(palette[currentVariable]);
    ...
    g.selectAll("path.county")
      ...
      .enter().append("path")
        ...
        .attr("d", path)
        .attr("fill", d =>
          Number.isFinite(d.properties[currentVariable])
            ? colorScale(d.properties[currentVariable])
            : "#ccc"
        )
        .attr("stroke", "white")
        ...
        .on("mouseover", function(event, d) {
          ...
          // build tooltip HTML
          let html;
          if (currentVariable === "walkability_index") {
            html = `
            ...
            const displayValue = Number.isFinite(d.properties[currentVariable])
              ? d.properties[currentVariable].toFixed(3)
              : "N/A";
            html = `
              <strong style="font-size:15px;">
                ${d.properties.county_name || "Unknown"} County
              </strong><br/>
              <strong>${title}:</strong> ${displayValue}<br/>
            `;
          }
          d3.select("#tooltip")
            .style("display", "block")
    ...
    states.forEach(s => {
      s.properties.state_name = stateIdToAbbr(s.id);
    });
    ...
    const legendNode = Legend(
      d3.scaleSequential(extent, palette[currentVariable]),
      { title }
    );
    ...
    // a) single <g> (g) is our zoom target
    // b) define zoom behavior
    const zoom = d3.zoom()
      .scaleExtent([1, 8])
    ...
    // c) compute the "home" transform (just margins, scale=1)
    const home = d3.zoomIdentity
      .translate(margin.left, margin.top)
      .scale(1);
  
    // d) bind zoom to this SVG, seed initial state, disable built-in dblclick
    svg.call(zoom)
       .call(zoom.transform, home)
       .on("dblclick.zoom", null)
       .on("dblclick", () => {  // double-click resets to full view
         svg.transition()
            .duration(750)
            .call(zoom.transform, home);
       });
  
    // e) click-to-focus handler
    function clicked(event, d) {
      event.stopPropagation();
  
      // compute bounding box of the clicked state
  ...
  async function init() {
    const walkability_data = await d3.csv("./data/walkability_by_county.csv", d => ({
      state_id: +d.state_fp,
    ...
    const map_geo = await d3.json("./data/us.json");
  
    walkability_data.forEach(v => {
      fips.forEach(f => {
    ...
    // 3) render each chart
    createVis(data, map_geo, "intersection_density", "Intersection Density",                  "#density");
    createVis(data, map_geo, "proximity_to_transit", "Proximity to Transit",                  "#proximity");
    createVis(data, map_geo, "employment_mix",       "Diversity of Land Use: Employment Mix", "#employment");
    createVis(data, map_geo, "household_mix",        "Diversity of Land Use: Household Mix",  "#household");
    createVis(data, map_geo, "walkability_index",    "Walkability Index",                     "#walkability");
  }
